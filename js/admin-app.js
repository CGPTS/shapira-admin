...
const EVENT_TYPES = Object.freeze([
  'בר מצווה','בת מצווה','ברית','אירוסין/נישואין',
  'יום הולדת','אירוע קהילתי','אזכרה','שבת חתן','אחר',
]);

/**
 * Canonicalize event-type labels for compatibility with backend rules.
 * Supports old typo "אירוע קהלתי" from legacy records/forms.
  ...
  filtered.forEach(b => {
    ...
    row.innerHTML = `
      ...
      <div class="booking-info" onclick="openBookingModal('${b.id}')">
        ...
        <div class="booking-badges">${depBadge}${countdownBadge}${b.projector ? '<span class="dep-badge dep-extra">🎥 הגברה + מסך ומקרן</span>' : ''}</div>
      </div>
...
function openBookingModal(bookingId) {
  ...
  document.getElementById('modalBody').innerHTML = `
    ...
    ${countdownHtml}
    <div class="modal-detail"><strong>🎥 הגברה + מסך ומקרן:</strong> ${b.projector ? 'כן (+₪200)' : 'לא'}</div>
...
async function updateBookingStatus(bookingId, newStatus) {
  ...
  try {
    ...
    Toast.error(err?.message === 'SLOT_TAKEN'
      ? 'לא ניתן לשחזר — המשמרת נתפסה בינתיים'
      : 'שגיאה בעדכון ההזמנה');
  }
}
...
async function submitEditBooking() {
  ...
  try {
    ...
    Toast.error(err?.message === 'SLOT_TAKEN'
      ? 'המשמרת החדשה נתפסה — בחרו תאריך/משמרת אחרים'
...
function exportCSV() {
  ...
  const cols = [
    ...
    'תאריך','משמרת','הגברה + מסך ומקרן','מחיר','פיקדון שולם','סטטוס','הערות',
  ];
...
function calculatePrice() {
  ...
  const base  = eventType === MEMORIAL_EVENT_TYPE
    ? MEMORIAL_EVENT_PRICE
    : (PRICING_TABLE[resident]?.[slot] ?? 0);
  const total = base + (hasProjector ? PROJECTOR_PRICE : 0);
  if (priceEl) priceEl.value = total;
  if (bd) {
    if (eventType === MEMORIAL_EVENT_TYPE) {
      bd.textContent = hasProjector
        ? `תעריף אזכרה שבועי ₪${MEMORIAL_EVENT_PRICE.toLocaleString()} + הגברה + מסך ומקרן ₪${PROJECTOR_PRICE} = ₪${total.toLocaleString()}`
        : `תעריף אזכרה שבועי ₪${total.toLocaleString()}`;
    } else {
      bd.textContent = hasProjector
        ? `₪${base.toLocaleString()} + הגברה + מסך ומקרן ₪${PROJECTOR_PRICE} = ₪${total.toLocaleString()}`
...
async function submitNewEvent() {
  ...
  try {
    ...
    Toast.error(err?.message === 'SLOT_TAKEN'
      ? 'המשמרת נתפסה זה עתה — רעננו ובדקו זמינות'
      : 'שגיאה בהוספת האירוע');
  } finally {
    _isSubmitting = false;
